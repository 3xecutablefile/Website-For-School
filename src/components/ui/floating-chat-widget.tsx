...
const AI_AGENTS: Agent[] = [
  { id: "gpt4", name: "GPT-4", role: "Advanced Reasoning", avatar: "https://images.unsplash.com/photo-1677442136019-21780ecad995?w=100&h=100&fit=crop", status: "online", icon: Sparkles, gradient: "from-green-500/20 to-emerald-500/20" },
  { id: "claude", name: "Claude 3.5", role: "Creative Writing", avatar: "https://images.unsplash.com/photo-1684392059507-2f719e6e7d2d?w=100&h=100&fit=crop", status: "online", icon: Brain, gradient: "from-orange-500/20 to-amber-500/20" },
  { id: "gemini", name: "Gemini Pro", role: "Multimodal Analysis", avatar: "https://images.unsplash.com/photo-1633419461186-7d40a381b2c2?w=100&h=100&fit=crop", status: "busy", icon: Zap, gradient: "from-blue-500/20 to-cyan-500/20" },
  { id: "copilot", name: "Copilot", role: "Code Assistant", avatar: "https://images.unsplash.com/photo-1618477388954-7852f32655ec?w=100&h=100&fit=crop", status: "online", icon: Code, gradient: "from-purple-500/20 to-violet-500/20" },
]
...
export function FloatingChatWidget() {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-4">
      <AnimatePresence>
        {isOpen && (
          <motion.div key="chat-window" variants={containerVariants} initial="hidden" animate="visible" exit="exit" className="w-[380px] overflow-hidden rounded-2xl border border-border/40 bg-background/60 shadow-2xl backdrop-blur-xl ring-1 ring-white/10">
              ...
              <div className="relative flex items-center justify-between z-10">
                <div className="flex items-center gap-3">
                  <div className="relative">
                    ...
                    <span className={cn("absolute bottom-0 right-0 h-3 w-3 rounded-full border-2 border-background", currentAgent.status === "online" ? "bg-emerald-500" : currentAgent.status === "busy" ? "bg-amber-500" : "bg-slate-400")} />
                  </div>
                  <div>
            ...
            <div className="border-b border-border/40 p-3">
              <Select value={selectedAgent} onValueChange={setSelectedAgent}>
                <SelectTrigger className="w-full border-none bg-transparent shadow-none focus:ring-0 text-lg font-medium h-auto px-2 py-6 cursor-pointer">
            ...
            <div className="border-t border-border/40 bg-background/60 p-3 backdrop-blur-md">
              <form className="relative flex items-center gap-2" onSubmit={(e) => { e.preventDefault(); setMessage("") }}>
                <input type="text" value={message} onChange={(e) => setMessage(e.target.value)} placeholder={`Message ${currentAgent.name}...`} className="flex-1 rounded-full border border-border/40 bg-background/50 px-4 py-2.5 text-sm outline-none transition-all placeholder:text-muted-foreground focus:border-primary/50 focus:ring-2 focus:ring-primary/10" />
                <Button size="icon" className="h-10 w-10 rounded-full bg-primary text-primary-foreground shadow-lg transition-transform hover:scale-105" disabled={!message.trim()}>
      ...
      <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={toggleOpen} className={cn("cursor-pointer group relative flex h-14 w-14 items-center justify-center rounded-full shadow-2xl transition-all duration-300", isOpen ? "bg-destructive text-destructive-foreground rotate-90" : "bg-primary text-primary-foreground hover:shadow-primary/25")}>
